...
@section('content')
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    @if(isset($attac) && !empty($attac))
                                        <a href="{{ getBaseURL().$attac->path }}" target="_blank" class="btn ripple btn-outline-primary btn-block">
                                ...
                                <select name="certification_id[]" class="form-control select2"  data-parsley-errors-container="#error_certification_id" multiple="true">
                                    @if(isset($certification) && !empty($certification))
                                        @foreach($certification as $ckey => $cvalue)
                                            @php $selected = ''; @endphp
                                            @if (in_array($cvalue->id,  (array) $selected_certificate)) @php $selected = 'selected'; @endphp @endif
                ...
                <div class="card custom-card">
                    <div class="card-body">
                        <!-- <div class="row">
                            <div class="col-12 col-sm-12">
                            <div class="form-group">
                                <label>{{ translate('Silder images') }} (<a href="javascript:void(0);" data-url="{{ getBaseURL().'filemanager/dialog.php?type=1&field_id=multi_slider_image_field' }}" class="btn-iframe text-danger" data-original-title="{{ translate('Click on the image to change') }}" data-placement="top" data-toggle="tooltip" title="{{ translate('Click on the image to change') }}">{{ translate('select') }}</a>)</label>
                            </div>
                            </div>
                        </div>
                        <div class="row image-sortable slider_images_row">
                            @php $i=0; $slider_images = json_decode(isset($product['slider_images']) && $product['slider_images']!= '' ? $product['slider_images'] : ''); @endphp
                            @if(isset($slider_images) && !empty($slider_images)) 
                                @foreach ($slider_images as $key => $value) 
                                    <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12 image-draggable" id="sr{{ $i }}" style="float:left;">
                                        <div class="form-group text-left">
                                            <img class="img-thumbnail" src="{{ uploads_url($value) }}">
                                            <input type="hidden" name="slider_images[]" value="{{$value}}">
                                            <a href="javascript:void(0);" class="btn btn-danger btn-block btn-remove-images" data-id="{{ $i }}"><i class="fa fa-times"></i>{{ translate('remove') }}</a>
                                        </div>
                                    </div>
                                @php $i++ @endphp
    ...
    <input type="hidden" id="multi_slider_image_field" value="">
    ...
    <script>
        function responsive_filemanager_callback(field_id){
            ...
            }else if(field_id=='multi_slider_image_field'){
                var img_url = $("#multi_slider_image_field").val();
                var img_cnt = $("#img_cnt").val();img_cnt++;$("#img_cnt").val(img_cnt);
                var html = '<div class="col-lg-2 col-md-2 col-sm-12 col-xs-12 image-draggable" id="ir'+img_cnt+'" style="float:left;">';
                    html += '   <div class="form-group text-left">';
                    html += '      <img class="img-thumbnail" src="'+img_url+'">';
                    html += '      <input type="hidden" name="slider_images[]" value="'+img_url+'">';
                    html += '      <a href="javascript:void(0);" class="btn btn-danger btn-block btn-remove-images" data-id="'+img_cnt+'"><i class="fa fa-times"> remove</i></a>';
                    html += '   </div>';
                    html += '</div>';
                $(".slider_images_row").append(html);
            }else{
